...
初期状態はローカルの既存サムネ `assets/youtube-st6-thumb.jpg` を使います。

動画IDを `site-config.json` の `youtube.fallbackVideos` に入れると、`https://i.ytimg.com/vi/<VIDEO_ID>/hqdefault.jpg` の形式でサムネを自動表示できます。

完全な「最新動画の自動反映」は、静的HTMLだけだとYouTube RSSのCORS制限を受けるため、`youtube.latestJsonUrl` にRSS変換APIや自前プロキシのJSON URLを入れる設計にしています。
