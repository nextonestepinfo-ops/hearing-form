...
const SCENE_ORDER = { moon: 0, mist: 1, live: 2, night: 3, bloom: 4, silver: 5 };
const SECTION_CONTENT_DELAY = reduceMotion ? 0 : 280;
const SCENE_SWITCH_DURATION = reduceMotion ? 0 : 760;
...
function setupReveal() {
  ...
  targets.forEach((node, index) => {
    node.style.setProperty("--reveal-delay", `${Math.min(index * 34, 170)}ms`);
  });
...
function setMood(nextMood, section) {
  if (!nextMood || nextMood === activeMood) return;
  const nextScene = section?.dataset?.scene || SCENE_BY_MOOD[nextMood] || "moon";
  ...
  if (nextScene !== currentScene && !reduceMotion) {
    ...
    const direction = (SCENE_ORDER[nextScene] ?? 0) >= (SCENE_ORDER[currentScene] ?? 0) ? "forward" : "back";
    body.dataset.sceneDirection = direction;
    body.classList.remove("is-scene-switching");
    void body.offsetWidth;
    body.classList.add("is-scene-switching");
    sceneTimer = window.setTimeout(() => body.classList.remove("is-scene-switching"), SCENE_SWITCH_DURATION);
  }
...
function updateScrollState() {
  ...
  root.style.setProperty("--v7-scroll", ratio.toFixed(4));
  ...
  if (current) setMood(moodForSection(current), current);
